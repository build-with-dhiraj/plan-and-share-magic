...
CREATE INDEX idx_quiz_attempts_type ON public.quiz_attempts (quiz_type, completed_at DESC);
...
CREATE INDEX idx_quiz_answers_question ON public.quiz_answers (user_id, question_id);
...
CREATE INDEX idx_daily_completions_date ON public.daily_completions (challenge_date DESC, total_xp DESC);
...
CREATE TABLE public.spaced_cards (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  question_id TEXT NOT NULL,
...
CREATE INDEX idx_spaced_cards_review ON public.spaced_cards (user_id, next_review);
